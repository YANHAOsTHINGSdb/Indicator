...
void Parse1(int nCount, float *pOut, float *pHigh, float *pLow)
{
  ...
  for (int i = 1; i < nCount; i++)
  {
    ...
    // 寻找低点模式
    //  0000000-10000000-100000000-1
    else if (nState == -1)
    {
      // 如果当前最低（pLow[i]）小于之前最低（pLow[nLow]），更新位置信息（-1）
      if (pLow[i] <= pLow[nLow])
      {
        pOut[nLow] = 0;
        nLow = i;
  ...
  for (int i = 0; i < nCount; i++)
  {
    ...
    if (pOut[i] == 1)
    {
          ...
          (pLow [nCurrBot] >  pLow [nPrevBot]))
      {
        // 检查合法性（严格按照连续五根形成一笔）
        //
        // （如果【4】与【3】间的单位不足4个，且【4】之后还有4个单位以上数据）
        //   或
        // （如果【3】与【2】间的单位不足4个单位数据）
        //   或
        // （如果【2】与【1】间的单位不足4个单位数据）
        if (((nCurrTop - nCurrBot < 4) && (nCount   - nCurrTop > 4)) ||
             (nCurrBot - nPrevTop < 4) || (nPrevTop - nPrevBot < 4))
        {
          pOut[nCurrBot] = 0;// 将【3】抹平
          pOut[nPrevTop] = 0;// 将【2】抹平
        }
        else if (nCount - nCurrTop > 4)
        {
          // 检查第三段（上）【3】【4】K线合并
          nSpan = nCurrTop - nCurrBot; // 计算最后一对高低点间的单位数
          // 遍历最后一对高低点间的数据
          // (如果存在喇叭形缩口，单位数减1，即无效波动)
